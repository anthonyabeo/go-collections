...
func (v *vertex) Item() interface{} {
	return v.item
}

func (v *vertex) OutgoingEdges() []*edge {
	return v.outgoingEdges
}

func (v *vertex) IncomingEdges() []*edge {
...
func NewEdgeWithDefaultData() *edge {
	return &edge{
		0,
		"sugar",
		new(vertex),
...
func (dalg *DirectedAdjacencyListGraph) PrimaryStructure() map[int]*vertex {
	return dalg.primaryStructure
}
...
// fromVertex (vertex): the vertex this edge starts from. The outgoing vertex.
// toVertex (vertex): the vertex this edge is incident on. The incoming vertex.
func (dalg *DirectedAdjacencyListGraph) GetEdge(fromVertexID, toVertexID int) (*edge, error) {
...
func (dalg *DirectedAdjacencyListGraph) InsertEdge(fromVertexID, toVertexID int, item interface{}) (*edge, error) {
	if e, _ := dalg.GetEdge(fromVertexID, toVertexID); e == nil {
		fromVertex := dalg.primaryStructure[fromVertexID]
		toVertex := dalg.primaryStructure[toVertexID]
		if fromVertex != nil && toVertex != nil {
			e := &edge{
	...
	return nil, errors.New("cannot insert edge")
}
